...
opt.read_options(opts, 'keep_session', function() end)
...
if opts.session_file == '' then
  ...
  if not watch_later:find('[/\\]$') then
    watch_later = watch_later .. '/'
  end

  opts.session_file = watch_later .. 'prev-session'
end
local save_file = mp.command_native({ 'expand-path', opts.session_file })
...
local function load_prev_session(file, load_playlist)
  ...
  if load_playlist == 'yes' then
    load_playlist = true
  elseif load_playlist == 'no' then
    load_playlist = false
  else
    load_playlist = opts.load_playlist
  end
...
if opts.auto_load and (mp.get_property_number('playlist-count', 0) == 0) then
  local function temp()
    load_prev_session()
